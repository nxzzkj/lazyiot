...
namespace Modbus.ModbusService
{
    ...
    ///  Modbus UDP 通讯
    /// </summary>
    public class Modbus_UDP : ScadaCommunicateKernel
    {
        ...
        protected override void Start()
        {
            ...
            if (udpClient == null)
                udpClient = new UdpClient(new IPEndPoint(IPAddress.Parse(Udp_PARA.LocalTCP_IP), int.Parse(Udp_PARA.LocalTCP_Port)));
        
            master = ModbusIpMaster.CreateIp(udpClient);
            //通用部分设置
            master.Transport.ReadTimeout = Udp_PARA.ReadTimeout;//读取数据超时1000ms
            master.Transport.WriteTimeout = Udp_PARA.WriteTimeout;//写入数据超时1000ms
            master.Transport.Retries = Udp_PARA.CollectFaultsNumber;//重试次数
            master.Transport.WaitToRetryMilliseconds = Udp_PARA.CollectFaultsInternal;//重试间隔     
        ...
        protected override void Close()
        {
            ...
            {
                this.DeviceException("ERROR=Modbus_Udp_10006," + emx.Message);
        ...
        protected override void Continue()
        {
            ...
            {
                this.DeviceException("ERROR=Modbus_Udp_10003," + emx.Message);
        ...
        protected override void Pause()
        {
            ...
            {
                this.DeviceException("ERROR=Modbus_Udp_10005," + emx.Message);
        ...
        private UdpClient _simularorClient;
        //模拟器初始化
        ...
        public override void SimulatorClose()
        {
            ...
            try
            {
                ...
                {
                    _simularorClient.Close();
                    _simularorClient = null;
                }
            }
            catch (Exception emx)
            {
