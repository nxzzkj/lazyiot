...
namespace Modbus.ModbusService
{
    ...
    /// 实现IOT TCP
    /// </summary>
    public class Nb_IotModbusServer: ScadaCommunicateKernel
    {
        ...
        protected override bool InitCommunicateKernel(IO_SERVER server, IO_COMMUNICATION communication, List<IO_DEVICE> ioDevices, SCADA_DRIVER driver)
        {
            ...
            if (communication != null)
            {
                ...
                //连接超时
                dtuConfig.Timeout = int.Parse(TcpParaPack.GetValue("连接超时"));
                ...
                for (int i = 0; i < this.IODevices.Count; i++)
                {
                    ...
                    ModbusFragmentStore fragment = new ModbusFragmentStore();
                    RealData mRealData = new RealData();
                    mRealData.Device = this.IODevices[i];
                    mRealData.Identification = devicePack.GetValue("注册标识");
                    ushort fixUnit = ushort.Parse(devicePack.GetValue("指定寄存器范围"));
                    ushort startUnit = ushort.Parse(devicePack.GetValue("起始地址"));
                    ushort unitNumber = ushort.Parse(devicePack.GetValue("寄存器数量"));
                    fragment.FixUnit = fixUnit;
                    fragment.FixStartUnit = startUnit;
                    fragment.FixUnitNumber = unitNumber;
                    ScadaDeviceKernel driverDll = DeviceDrives.Find(x => x.DeviceDriverID == this.IODevices[i].DEVICE_DRIVER_ID);
                    fragment.DevAddress = Convert.ToByte(this.IODevices[i].IO_DEVICE_ADDRESS);
                    if (driverDll != null)
                    {
                    ...
                    //进行分段,保证所有数据都能获取到
                    mRealData.Fragment.MakeFragment();
                        ...
                        try
                        {
                            ...
                            if (item != null && item.RefreshTime != null)
                            {
                                if (item.IsOnline && item.RefreshTime != null)
                                {
                                    ...
                                    if (span.TotalSeconds > 60*10)//10分钟后如果没有任何刷新数据,则判断此连接断开
                                    {
                                        list.Add(item);
                                    }
        ...
        protected override void Start()
        {
            try
            {
                ...
                if (this.dtuConfig.ReceiveType == ReceiveType.主动)//标识主动向每个客户端询问请求
                {
                    activeTask= Task.Run(() =>
                    {
                        while (true)
                        {
                            if (this.ServerIsSuspend)
                                continue;
                            foreach (RealData realData in RealDevices)
                            {
                                //当前设备数据的过期需要重新更新
                                if (realData.LastTime.AddSeconds(realData.Device.IO_DEVICE_UPDATECYCLE) <= DateTime.Now)
                                {
                                    try
                                    {
                                        Task.Run(() =>
                                        {
                                            string error = "";
                                            for (int i = 0; i < realData.ReadSendByte.Count; i++)
                                            {

                                                this.Send(RealDevices[i].Identification, realData.ReadSendByte[i], out error, true);
                                            }
        ...
        private void svr_OnDataIn(object sender, ZYBDataInEventArgs e)
        {
            if (((sender != null) && (e != null)) && ((e.Data != null) && (e.Data.Length >= 0)))
            {
                try
                {
                    ...
                    {
                        //向用户发送数据
                        RealData realdevice = RealDevices.Find(x => x.Identification == Identification);
                        if (realdevice != null)
                            this.ReceiveData(this.IOServer, this.IOCommunication, realdevice.Device, s, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), realdevice.Fragment);
                        return;
                    }
