...
{
    public class ModbusNbIot_DeviceDriver : ScadaDeviceKernel
    {

        private const string mGuid = "9DBD5881-DF9B-4816-84DF-484181532C70";
        /// <summary>
        /// 驱动唯一标识，采用系统GUID分配
        /// </summary>
        ...
        protected override void DeviceDriveDispose()
        {
            base.DeviceDriveDispose();
        }
        protected override bool InitDeviceKernel(IO_SERVER server, IO_COMMUNICATION communication, IO_DEVICE device, IO_PARA para, SCADA_DEVICE_DRIVER driver)
        {
            if (IsCreateControl)
            {
                if (para != null)
                {
                    this.ParaCtrl = new ModbusNbIotParaCtrl();
                    this.ParaCtrl.SetUIParameter(server, device, para);


                }
                this.DeviceCtrl = new NbIotDeviceCtrl();
