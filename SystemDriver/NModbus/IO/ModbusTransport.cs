namespace Modbus.IO
{
    ...
    public abstract class ModbusTransport : IDisposable
    {
        ...
        ///     遇到失败（如IOException）后重试发送消息的次数
        ///     TimeoutException, or a corrupt message.TimeoutException或损坏的消息
        /// </summary>
        public int Retries
        {
            get { return _retries; }
        ...
        ///如果非零，如果第一个在
        ///小于此数字的请求。例如，将其设置为3，如果发送请求5时，响应3为
        ///阅读，我们将尝试重新阅读回答。
        /// </summary>
        ...
        /// 如果已设置，则从机忙异常将导致使用重试计数。如果为false，从机繁忙将导致无限次重试
        /// </summary>
