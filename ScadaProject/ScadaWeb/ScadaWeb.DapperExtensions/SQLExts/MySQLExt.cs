...
namespace DapperExtensions.MySQLExt
{
    public static class MySQLExt
    {
            ...
            { 
               ...
               //*******************插入数据**************************************//
                switch (sqls.KeyType)
                {
                    ...
                    case "Int32": return conn.ExecuteScalar<int>(sqls.InsertSql + ";select " +sqls.KeyName+" from "+sqls.TableName+" order by id desc limit 1;", entity, transaction, commandTimeout); //int
                    case "Int64": return conn.ExecuteScalar<long>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //long
                    case "Decimal": return conn.ExecuteScalar<decimal>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //decimal
                    case "UInt32": return conn.ExecuteScalar<uint>(sqls.InsertSql+ ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //uint
                    case "UInt64": return conn.ExecuteScalar<ulong>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //ulong
                    case "Double": return conn.ExecuteScalar<double>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //double
                    case "Single": return conn.ExecuteScalar<float>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //float
                    case "Byte": return conn.ExecuteScalar<byte>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout);  //byte
                    case "SByte": return conn.ExecuteScalar<sbyte>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //sbyte
                    case "Int16": return conn.ExecuteScalar<short>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //short
                    case "UInt16": return conn.ExecuteScalar<ushort>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //ushort
                    default: return conn.ExecuteScalar<dynamic>(sqls.InsertSql + ";select " + sqls.KeyName + " from " + sqls.TableName + " order by id desc limit 1;", entity, transaction, commandTimeout); //dynamic
                }
            }
            else
            {
        ...
        //直接执行一条sql语句
        private static IEnumerable<T> GetBySql<T>(this IDbConnection conn, Type t, string sql, object param = null,IDbTransaction transaction = null, int? commandTimeout = null)
        {
        ...
        public static IEnumerable<T> GetBySql<T>(this IDbConnection conn, string sql, object param = null)
        {
            return GetBySql<T>(conn, typeof(T), sql, param);
        }
        /// <summary>
        /// 根据查询条件获取数据
        /// </summary>
