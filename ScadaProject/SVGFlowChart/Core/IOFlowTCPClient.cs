...
namespace ScadaFlowDesign.Core
{
    ...
    public  class IOFlowTCPClient : ScadaTask
    {
        ...
        private async Task RunAsync(int port, IPAddress remoteIp)
        {
            ...
            Client = new AsyncTcpClient
            {
                ...
                Port = port,
                AutoReconnect = true,
                 ScadaClientType= ScadaClientType.FlowDesign,
        ...
        private void SplitReceivePack(byte[] receivebytes, int count, AsyncTcpClient c)
        {
            ...
            if (c.IsOperator(opeartor))
            {
                switch (operatorEnum)
                {
                    case ScadaTcpOperator.流程设计器登录反馈:
                        {
                            try
                            {
                                #region 处理采集器端登录
                        ...
                        break;
                    case ScadaTcpOperator.流程发布请求反馈:
                        {
                            try
                            {
                                #region 流程发布请求反馈
                                ...
                                try
                                {
                                    ...
                                    if (RESULT == "true")
                                    {
                                        if (OnPublishProject != null)
                                        {
                                            OnPublishProject(c, true, MSG, PROJECTID);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (OnUserLogined != null)
                                        {
                                            OnPublishProject(c, false, MSG,PROJECTID);
                                        }
                                    }
                                }
                                catch
                        ...
                        break;
                    case ScadaTcpOperator.流程发布失败://接收到服务器端更新采集站中报警配置通知
                        {
                            TcpData tcpData = new TcpData();
                            byte[] narra = new byte[count - 1];
                        ...
                        break;
                    case ScadaTcpOperator.流程发布成功://接收到服务器端更新采集站中报警配置通知
                        {
                            TcpData tcpData = new TcpData();
                            byte[] narra = new byte[count - 1];
                            ...
                            {
                                OnPublishProjectSuccess(c, true, tcpData.GetItemValue("MSG"),"");
                            }

                        }
                        break;
                        
                    case ScadaTcpOperator.流程发布进度:
                        {
                        ...
                        break;
                    case ScadaTcpOperator.流程发布传输进度:
                        {
        ...
        public  async void Start(string IP)
        {
            ...
            await RunAsync(int.Parse(FlowManager.Config.RemotePort), IPAddress.Parse(IP.Trim().Replace("。",".")));
