namespace Modbus.IO
{
    ...
    public abstract class ModbusTransport : IDisposable
    {
        ...
        /// <summary>
       ///遇到IOException等故障后重试发送消息的次数，
        ///TimeoutException或损坏的消息。
        ...
        /// <summary>
        ///如果非零，则如果第一个应答在序列号之后，则将导致读取第二个应答
        ///请求少于这个号码。例如，将其设置为3，如果在发送请求5时，响应3是
        ///读，我们将尝试重新读取响应。
        ...
        /// <summary>
        /// 如果设置，从机忙异常将导致使用重试计数。如果为false，Slave Busy将导致无限次重试
