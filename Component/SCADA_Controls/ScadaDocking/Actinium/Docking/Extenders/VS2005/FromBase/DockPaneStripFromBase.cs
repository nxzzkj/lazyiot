...
namespace GraphicsUI.DockControl.Docking.Extenders.VS2005
{
	...
	public class DockPaneStripFromBase : DockPaneStripBase
	{
		...
		private const int _DocumentTabGapLeft = 5;
		private const int _DocumentTabGapRight = 5;
		private const int _DocumentTextExtraHeight = 6;
		private const int _DocumentTextExtraWidth = 20;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			using (LinearGradientBrush brush = new LinearGradientBrush(rect, ColorMixer.DockPaneStripBackDarkColor, ColorMixer.DockPaneStripBackLightColor, LinearGradientMode.Horizontal))
			{
		...
		private void DrawTabStrip_Document(Graphics g)
		{
			...
			g.DrawLine(OutlineOuterPen
				, rectTabStrip.Left, rectTabStrip.Bottom - 1,
			...
			int currentIndex = Tabs.IndexOf(DockPane.ActiveContent);
			...
			//do the current one last because of its design, it overlaps the others
			rectTab = GetTabRectangle(currentIndex);
			if (rectTab.IntersectsWith(rectTabOnly))
				DrawTab(g, Tabs[currentIndex].Content, rectTab);
		}
