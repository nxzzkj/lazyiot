	...
	public class DockPaneStripFromBase : DockPaneStripBase
	{
		...
		private void DrawTab_ToolWindow(Graphics g, DockContent content, Rectangle rect)
		{
			...
			if (DockPane.ActiveContent == content)
			{
				...
				DrawUtil.DrawTab(g,rect,Corners.RB,GradientType.Flat,ColorMixer.DarkColor,ColorMixer.LightColor,Color.Black,false);
...
//					rect.X + rect.Width - 1, rect.Y, rect.X + rect.Width - 1, rect.Y + rect.Height - 1);
				g.DrawString(content.TabText, Font, new SolidBrush(ColorMixer.LightColor), rectText, ToolWindowTextStringFormat);
			}
			else
			{
