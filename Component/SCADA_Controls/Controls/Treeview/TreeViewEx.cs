...
using Scada.Controls.Properties;
...
namespace Scada.Controls.Controls
{
    ...
    {
        [Category("自定义属性"), Description("用户鼠标点击节点事件）")]
        public event TreeNodeMouseClickEventHandler OnTreeNodeMouseClickEX;
        /// <summary>
        /// The ws vscroll
        /// </summary>
        ...
        {
            if (e.Node.Tag != null && e.Node.Tag.ToString() == "0")
                return;
           
            //此处代码需要修改，该事件导致用户点击两次这个事件
            try
            {
                if (e.Node != null)
                {
                    ...
                    if (base.SelectedNode != null)
                    {
                        ...
                        if (OnTreeNodeMouseClickEX != null)
                        {
                            OnTreeNodeMouseClickEX(sender, e);
                        }
                    }
                }
            }
        ...
        private void treeview_DrawNode(object sender, DrawTreeNodeEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    else if (e.Node.Tag != null && e.Node.Tag.ToString() == "0")
                    {
                        e.Graphics.FillRectangle(new SolidBrush(_nodeBackgroundColor), new Rectangle(new Point(0, e.Node.Bounds.Y), new Size(base.Width, e.Node.Bounds.Height)));
                        e.Graphics.DrawString(e.Node.Text, font, new SolidBrush(Color.Red), (float)e.Bounds.X + intLeft, (float)e.Bounds.Y + ((float)this._nodeHeight - this.treeFontSize.Height) / 2f);
                    }
