...
namespace Scada.Controls.Controls
{
    ...
    public class UCRollText : UserControl
    {
        ...
        void m_timer_Tick(object sender, EventArgs e)
        {
            if (rectText == Rectangle.Empty)
                return;
            if (_RollStyle == Scada.Controls.Controls.RollStyle.BackAndForth && rectText.Width >= this.Width)
            {
                return;
            }
            rectText.X = rectText.X + _moveStep * m_intdirection;
            if (_RollStyle == Scada.Controls.Controls.RollStyle.BackAndForth)
            {
                if (rectText.X <= 0)
                {
                    m_intdirection = 1;
            ...
            else if (_RollStyle == Scada.Controls.Controls.RollStyle.LeftToRight)
            {
        ...
        {
            if (_RollStyle == Scada.Controls.Controls.RollStyle.LeftToRight)
            {
                m_intdirection = 1;
                if (rectText != Rectangle.Empty)
                {
            ...
            else if (_RollStyle == Scada.Controls.Controls.RollStyle.RightToLeft)
            {
                m_intdirection = -1;
                if (rectText != Rectangle.Empty)
                {
